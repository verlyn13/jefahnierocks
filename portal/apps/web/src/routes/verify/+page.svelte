<script lang="ts">
  ...
  async function handlePasskey() {
    ...
    // Production mode with real WebAuthn
    try {
  ...
  async function handleMagicLink() {
    ...
    if (demoMode && browser) {
      setTimeout(() => {
        alert("In demo mode: Check your email for a sign-in link!");
        loading = false;
      }, 500);
      return;
    }
...
<div class="min-h-screen bg-gradient-to-br from-glacier-900 to-black flex items-center justify-center relative overflow-hidden">
  ...
  <div class="relative z-10 w-full max-w-md p-8 bg-black/40 backdrop-blur-xl rounded-3xl border border-white/10">
    <div class="text-center mb-8">
      ...
      {#if demoMode}
        <p class="text-aurora-500 text-xs mt-2">(Demo Mode - Click to continue)</p>
      {/if}
    </div>
    ...
    <div class="space-y-4">
      ...
      >
        ...
        {loading ? "Authenticating..." : demoMode ? "Continue to Portal" : "Use Passkey"}
      </button>
